...
impl<T> List<T> {
    ...
    fn pop_front(&mut self) -> Option<T> {
        self.head.take().map(|old_head| {
            // in case we have next, make it head
            // [nil;n0] <-> [nn;n1] <-> [n0; n2] <-> [n1; nil]
            // [nil;n1] <-> [n0; n2] <-> [n1; nil]
            match old_head.borrow_mut().next.take() {
                Some(new_head) => {
                    new_head.borrow_mut().prev.take(); // similar to just = None
                    self.head = Some(new_head);
                }
                None => {
                    self.tail.take();
    ...
    fn pop_back(&mut self) -> Option<T> {
        self.tail.take().map(|old_tail| {
            match old_tail.borrow_mut().prev.take() {
                Some(new_tail) => {
                    new_tail.borrow_mut().next.take(); // similar to just = None
                    self.tail = Some(new_tail);
                }
                None => {
                    self.head.take();
